        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border-light dark:border-border-dark">
          <div>
            ...
            <p className="text-xs text-text-light-secondary dark:text-text-dark-secondary">
              {events.length} 个事件，{tasks.length} 个任务
              {timeEntries.length > 0 && `，${timeEntries.length} 条时间记录`}
                ...
                {tasks.map((task) => (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-text-light-secondary dark:text-text-dark-secondary mt-0.5">
                      状态：{task.status === 'backlog' ? '待处理' : task.status === 'todo' ? '待办' : task.status === 'inprogress' ? '进行中' : task.status === 'review' ? '评审中' : '已完成'}
                    </div>
                    {task.description && (
                      <p className="mt-1 text-[10px] text-text-light-tertiary dark:text-text-dark-tertiary line-clamp-2">
                        {task.description}
