...
test.describe('Widget Presets', () => {
  ...
  test('dashboard has settings gear', async ({ page }) => {
    const settingsBtn = page.locator('[aria-label*="页面设置"], [title*="页面设置"]').first();
  ...
  test('has preset options in settings', async ({ page }) => {
    ...
    if (await settingsBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const presetOption = page.getByText(/预设|布局/i).first();
  ...
  test('has customize widgets option', async ({ page }) => {
    ...
    if (await settingsBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const customizeOption = page.getByText(/自定义.*组件|管理.*组件/i).first();
